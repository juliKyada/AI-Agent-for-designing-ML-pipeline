   ...
   Fig.~\ref{fig:flowchart} illustrates this four-stage cascading process.
   ...
   \begin{tikzpicture}[
     node distance=0.5cm,
     proc/.style={rectangle, draw=black, fill=blue!12, rounded corners=3pt,
                  text width=4.2cm, minimum height=0.65cm, align=center, font=\small},
     modbox/.style={rectangle, draw=black!60, fill=orange!18, rounded corners=3pt,
                    text width=4.2cm, minimum height=0.55cm, align=center,
                    font=\scriptsize},
     arr/.style={->, >=stealth, thick}
   ]
     \node[proc] (inp)  {\textbf{Dataset Metadata}\\
                         ${(n,\,p,\,\text{feature types},\,\text{skewness},\ldots)}$};
     \node[proc, below=of inp]  (sz)   {\textbf{Step~1: Size Tier}\\
                                         {\scriptsize Tiny / Small / Medium / Large / Huge}};
     \node[proc, below=of sz]   (cx)   {\textbf{Step~2: Complexity Score}\\
                                         {\scriptsize Simple ($\leq\!2$) / Moderate (3--4) / Complex ($\geq\!5$)}};
     \node[proc, below=of cx]   (base) {\textbf{Step~3: Base Model Pool}\\
                                         {\scriptsize (Tables~\ref{tab:cls_rules} \&~\ref{tab:reg_rules})}};
     \node[modbox, below=0.7cm of base] (mod) {\textbf{Step~4: Modifier Rules 4--9}\\
                                         {\scriptsize Imbalanced $\Rightarrow$ inject \texttt{class\_weight=balanced}}\\
                                         {\scriptsize High-dim ($p/n>0.1$) $\Rightarrow$ promote regularised LR/Lasso}\\
                                         {\scriptsize Outlier density $>5\%$ $\Rightarrow$ promote tree models}\\
                                         {\scriptsize High cardinality $\Rightarrow$ promote tree models}};
     \node[proc, below=0.7cm of mod]  (out) {\textbf{Output:} Prioritised Top-$k$ Candidates\\
                                         + Hyperparameter Grids};
     \draw[arr] (inp)  -- (sz);
     \draw[arr] (sz)   -- (cx);
     \draw[arr] (cx)   -- (base);
     \draw[arr] (base) -- node[right, font=\scriptsize, text width=2.4cm, align=left]
                            {re-rank \& augment} (mod);
     \draw[arr] (mod)  -- (out);
   \end{tikzpicture}%
   }
   \caption{Rule-based model selection flowchart. Dataset metadata drives four sequential stages: size tiering, complexity scoring, base pool assembly (Tables~\ref{tab:cls_rules}--\ref{tab:reg_rules}), and modifier re-ranking. The output is a prioritised list of top-$k$ candidates with associated hyperparameter grids.}
   \label{fig:flowchart}
   \end{figure}
   ...
   Table~\ref{fig:metadata_heatmap} summarises the key metadata properties of all four datasets; highlighted cells indicate conditions that trigger MetaFlow modifier rules.
